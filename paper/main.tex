...
\title{Nine Percent Labelled: What Has to Be True\\
Before Self-Supervision Helps}
...
on a dataset where \SI{90.9}{\percent} of recorded sensor time carries no label
at all. The appeal was obvious: two hundred times more unlabelled than labelled
data, and a natural pretext task. We never got there, and this paper is an
...
improving on. Establishing both consumed the study. We show that the labels do
mark a physiological state, but in one channel only: skin conductance rises at
reported onset in \SI{64.3}{\percent} of episodes ($p = 0.0009$), while heart
...
permutation null ($0.498 \pm 0.010$), yet recovers only \SI{10.6}{\percent} of
...
\SI{9.1}{\percent} of the \SI{1251.7}{\hour} of recording falls inside a rated
episode. The remaining \SI{90.9}{\percent} is unlabelled physiology, which is
...
native rates: acceleration at \SI{32}{\hertz}, blood volume pulse at
\SI{64}{\hertz}, electrodermal activity and temperature at \SI{4}{\hertz}, and
heart rate at \SI{1}{\hertz}, taken from the file headers. Published
descriptions giving \SI{72}{\hertz} and \SI{10}{\hertz} for pulse and
temperature disagree with the files themselves, and we follow the files.

The archive contains 609 sessions totalling \SI{1251.7}{\hour} across 250
person-days. The median session is \SI{1.17}{\hour} rather than a full shift,
...
(\SI{20.4}{\percent}); treating them as naive \texttt{America/Chicago} places
212 (\SI{86.5}{\percent}). A fixed five-hour offset reaches 198, so the
daylight-saving-aware zone is required: the archive spans April to December.
Thirty-three rated episodes (\SI{13.5}{\percent}) fall outside every session
...
\SI{120}{\second} sufficient for time-domain measures \cite{munoz2015}, with
root mean square of successive differences validated from recordings as short
as \SI{15}{\second} \cite{orini2023}; the classical five-minute figure is a
...
\SI{43.6}{\percent} positive by construction, whereas true window prevalence
over all worn windows is \SI{6.49}{\percent}, so counting alarms against the
...
prevalence-invariant, and translate: at \SI{6.49}{\percent} prevalence there
...
all. Four have median skin conductance between \SI{0.07}{} and
...
\SI{64.3}{\percent} of episodes, a shift that survives both a sign test and a
Wilcoxon signed-rank test ($p = 3.3 \times 10^{-6}$), and the raw median shift
of \SI{0.64}{\micro\siemens} sits on a baseline of \SI{0.68}{\micro\siemens},
roughly a doubling. Heart rate rises in \SI{48.6}{\percent} of episodes, which
...
\SI{63}{\percent} is contributed by five episodes out of 140. The cause is
...
roughly \SI{40}{\percent} of them, so the common \SI{1}{\hertz} grid is used
...
interval file is present in every session, only \SI{5.9}{\percent} of
\SI{120}{\second} windows contain a run of clean consecutive beats long enough
to compute it, and the archive holds only \SI{39.5}{\hour} of usable beat data
out of \SI{1251.7}{\hour}. Reported session-level availability of
\SI{48}{\percent} is eight times more flattering than the window-level figure
...
it recovers \SI{10.6}{\percent} of episodes, with a seed range of $0.072$ that
...
\textbf{What we have eliminated is the label-treatment axis, not the
...
\subsection{What this implies for the pretraining we did not run}
...
intervals were usable in under \SI{6}{\percent} of windows.
...
\SI{90.9}{\percent} of the data is unlabelled. Establishing the prerequisites
...
We report this as a result rather than a failure, and we are explicit about its
